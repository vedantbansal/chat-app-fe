    ...
    return (
        <div className='flex h-full flex-col'>
            ...
            <div className='w-full h-16 flex justify-center place-items-center py-1 px-3 '>
                <input className='border-y-2 border-l-2 border-slate-100 rounded-l-full pl-3 pr-2 h-5/6 w-full focus:outline-none font-font-serif' type='text'></input>
            ...
            <div className='overflow-y-auto h-11/12'>
                {friends.map((user) =>
                    <div key={user.id} id={user.id} className='cursor-pointer h-1/12 hover:bg-slate-200 mx-1 rounded-md'><UserCard user={user} /></div>
                )}
            </div>
        </div>
    )
